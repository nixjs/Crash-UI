...
export class Serialize {
  ...
  private isSpecialType(prop: object) {
    let propKeys = Object.keys(prop).filter(
      (k) => k != "type" && k != "digits" && k != "length" && k != "trim"
    );
        ...
        if (data.hasOwnProperty(property)) {
          if (typeof data[property] === "object") {
            ...
            if (Array.isArray(data)) {
              ...
              schema[property]?._type === "BitArray8" ||
              schema[property]?._type === "BitArray16"
            ) {
              flat.push({
          ...
          //---
            ...
            if (
              schema[property]?.type?._type &&
              this.isSpecialType(schema[property])
            ) {
  ...
  public fromBuffer(buffer: ArrayBuffer) {
    ...
    schemaIds.forEach((id, i) => {
      ...
      if (schemaId)
        schemas.push({
          id,
          schema: Lib._schemas.get(id),
          startsAt: indexes[i] + 5,
        });
    });
        ...
        for (let property in struct) {
          if (struct.hasOwnProperty(property)) {
            ...
            if (prop && prop["_type"] && prop["_bytes"]) {
              ...
              // const _removeSpace = prop["_removeSpace"];
              let value;
